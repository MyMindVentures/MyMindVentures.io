...
export const AppArchitecture: React.FC<AppArchitectureProps> = ({ onBack }) => {
  ...
  const [expandedNodes, setExpandedNodes] = useState<string[]>(['dashboards', 'app-management']);
  ...
  const renderTreeNode = (node: NavigationNode, index: number = 0) => {
    ...
    const hasChildren = node.hasSubmenu && node.submenu && node.submenu.length > 0;
    ...
    return (
      ...
      >
        ...
        >
          {/* Level Indicator */}
          <div className={`w-3 h-3 rounded-full bg-gradient-to-r ${levelColors[node.level as keyof typeof levelColors]}`} />
          
          {/* Icon */}
          <div className={`w-8 h-8 rounded-lg bg-gradient-to-r ${levelColors[node.level as keyof typeof levelColors]} flex items-center justify-center`}>
        ...
        <AnimatePresence>
          {hasChildren && isExpanded && (
            ...
            >
              {node.submenu?.map((subNode, subIndex) => renderTreeNode(subNode, subIndex))}
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
              ...
              >
                <div
                  className={`w-48 p-4 bg-gray-800/50 rounded-xl border border-gray-700/50 cursor-pointer transition-all duration-200 hover:border-cyan-500/50 ${
                    selectedNode === node.id ? 'border-cyan-400/50 bg-cyan-500/10' : ''
                  } ${node.status === 'coming-soon' ? 'opacity-60' : ''}`}
  ...
  const renderGridView = () => {
    const levelGroups = {
      ...
      1: navigationStructure.flatMap(n => n.submenu || []).filter(n => n.level === 1),
      2: navigationStructure.flatMap(n => n.submenu?.flatMap(s => s.submenu || []) || []).filter(n => n.level === 2),
    };
          ...
          return (
            <div key={level}>
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center">
                <div className={`w-3 h-3 rounded-full bg-gradient-to-r ${levelColors[level as keyof typeof levelColors]} mr-3`} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {nodes.map((node, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center space-x-3 mb-2">
                        <div className={`w-8 h-8 rounded-lg bg-gradient-to-r ${levelColors[level as keyof typeof levelColors]} flex items-center justify-center`}>
                          <Icon className="w-4 h-4 text-white" />
                        </div>
                        <span className="text-white font-medium text-sm">{node.label}</span>
                      </div>
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Pages', value: '14', icon: FileText, color: 'text-cyan-400' },
          { label: 'Menu Levels', value: '3', icon: GitBranch, color: 'text-purple-400' },
          { label: 'Active Tools', value: '6', icon: Zap, color: 'text-green-400' },
          { label: 'Coming Soon', value: '8', icon: Circle, color: 'text-orange-400' },
        ].map((stat, index) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <Card title={`Navigation Structure - ${viewMode.charAt(0).toUpperCase() + viewMode.slice(1)} View`} gradient>
            <div className="space-y-4">
              {viewMode === 'tree' && (
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {navigationStructure.map((node, index) => renderTreeNode(node, index))}
                </div>
              )}
        ...
        <div className="space-y-6">
            ...
            {selectedNodeDetails ? (
              <div className="space-y-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="text-white font-medium">{selectedNodeDetails.label}</h4>
                    <span className="text-xs text-gray-400">Level {selectedNodeDetails.level}</span>
                  </div>
                </div>
                ...
                {selectedNodeDetails.hasSubmenu && (
                  <div className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-lg">
                    <div className="flex items-center space-x-2 text-purple-400 text-sm">
                      ...
                      <span>Has {selectedNodeDetails.submenu?.length} submenu items</span>
                    </div>
                  </div>
                )}
              </div>
          ...
          <Card title="Architecture Metrics">
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <span className="text-gray-400 text-sm">Development Status</span>
                <span className="text-orange-400 font-medium">43% complete</span>
              </div>
            </div>
          </Card>
