...
export const GitManagement: React.FC<GitManagementProps> = ({ onBack }) => {
  const [activeTab, setActiveTab] = useState<
    'overview' | 'commits' | 'branches' | 'files'
  >('overview');
  ...
  const stageFile = async (filePath: string) => {
    try {
      ...
      setFileStatus(prev =>
        prev.map(f => (f.path === filePath ? { ...f, staged: true } : f))
      );
    } catch (error) {
        ...
        <div className='flex items-center space-x-4'>
          ...
          <div>
            ...
            <p className='text-gray-400'>
              Manage your codebase version control
            </p>
          </div>
        </div>
      ...
      <div className='bg-gray-800 rounded-lg p-4 mb-6'>
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center space-x-4 text-sm text-gray-400'>
            ...
            <span>
              {
                fileStatus.filter(f => !f.staged && f.status !== 'committed')
                  .length
              }{' '}
              unstaged
            </span>
