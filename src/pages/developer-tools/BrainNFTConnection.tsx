...
export const BrainNFTConnection: React.FC<BrainNFTConnectionProps> = ({ onBack }) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'contracts' | 'ventures' | 'analytics'>('overview');
  ...
  const [selectedVenture, setSelectedVenture] = useState<JointVenture | null>(null);
  ...
  const mockVentures: JointVenture[] = [
    {
      ...
      description: 'A revolutionary content creation platform powered by advanced AI',
      objectives: [
    ...
    <div className="min-h-screen bg-gray-900 text-white p-6">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-400">Manage AI partnerships and joint ventures</p>
          </div>
        </div>
      ...
      <div className="flex space-x-1 mb-6 bg-gray-800 rounded-lg p-1">
        {[
          ...
          { id: 'ventures', label: 'Joint Ventures', icon: Users },
            ...
            <Card className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{analytics.total_contracts}</p>
                </div>
            ...
            <Card className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{analytics.active_ventures}</p>
                </div>
            ...
            <Card className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-400">+{analytics.monthly_growth}%</p>
                </div>
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-400 text-sm">{contract.partner}</p>
                      </div>
                    </div>
                    <div className={`px-2 py-1 rounded-full text-xs ${
                      contract.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'
                    }`}>
                      {contract.status}
                    </div>
                  </div>

                  <p className="text-gray-300 mb-4">{contract.partner_description}</p>
            ...
            <div className="space-y-6">
              {mockVentures.map((venture) => (
                <Card key={venture.id} className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h3 className="text-xl font-semibold mb-2">{venture.venture_name}</h3>
                      <p className="text-gray-400">{venture.description}</p>
                    </div>
                    <div className={`px-3 py-1 rounded-full text-sm ${
                      venture.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'
                    }`}>
                      {venture.status}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                      ...
                      <p className="font-medium">{venture.partners.join(', ')}</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Revenue Split</p>
                      <p className="font-medium">{venture.revenue_sharing.mymindventures}% / {venture.revenue_sharing.partner}%</p>
                    </div>
                    <div>
                      <p className="text-gray-400 text-sm">Timeline</p>
                      <p className="font-medium">{format(new Date(venture.timeline.start_date), 'MMM yyyy')} - {format(new Date(venture.timeline.end_date), 'MMM yyyy')}</p>
                    </div>
                  </div>
              ...
              <Card className="p-6">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium">{analytics.partner_distribution.perplexity}%</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-gray-400">Bolt</span>
                    <span className="font-medium">{analytics.partner_distribution.bolt}%</span>
                  </div>
                </div>
              </Card>
              ...
              <Card className="p-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-green-400">+{analytics.monthly_growth}%</span>
                  </div>
                </div>
              </Card>
            </div>
