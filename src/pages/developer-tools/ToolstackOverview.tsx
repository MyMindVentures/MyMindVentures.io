...
export const ToolstackOverview: React.FC<ToolstackOverviewProps> = ({
  onBack,
}) => {
  ...
  const tools: Tool[] = [
    ...
    // AI & External Services
    {
      ...
      version: 'GPT-4',
      category: 'AI',
      description: 'AI models for intelligent features',
      icon: Cpu,
  ...
  const categoryStats = categories.slice(1).map(category => ({
    ...
    active: tools.filter(
      tool => tool.category === category && tool.status === 'active'
    ).length,
  }));
            ...
            >
              <div className='space-y-4'>
                <div className='flex items-start justify-between'>
                  ...
                  <div
                    className={`flex items-center space-x-1 px-2 py-1 rounded text-xs ${getStatusColor(tool.status)}`}
                  >
