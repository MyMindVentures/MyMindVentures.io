...
export const SettingsApp: React.FC<SettingsAppProps> = ({ onBack }) => {
  ...
  const loadAPIConnections = async () => {
    setIsLoading(true);
    try {
      // FORCE RESTORE: Always recreate default connections to ensure database integrity
      console.log('🔄 Force restoring default API connections...');

      // First, clear existing connections to avoid duplicates
      ...
      if (deleteError) {
        console.log('Note: Could not clear existing connections:', deleteError);
      }

      // Create fresh default connections with ACTUAL CREDENTIALS
      const defaultConnections = [
        {
          service_name: 'openai',
          status: 'connected',
          user_id: 'demo-user',
          api_key_encrypted: 'sk-...your-openai-api-key-here...',
        ...
        },
        {
          ...
          status: 'connected',
          user_id: 'demo-user',
          api_key_encrypted: '[SUPABASE_ANON_KEY]',
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        },
        {
          ...
          status: 'connected',
          user_id: 'demo-user',
          api_key_encrypted: '[GITHUB_PERSONAL_ACCESS_TOKEN]',
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        },
      ];
      ...
      console.log(
        ...
        defaultConnections.map(c => c.service_name)
      );
      ...
      for (const conn of defaultConnections) {
        ...
        if (error) {
          ...
          console.log(`✅ Created ${conn.service_name} connection`);
        }
      }

      // Load the fresh connections
      const { data } = await db.getAPIConnections('demo-user');
      if (data) {
        console.log('🔄 Loaded fresh connections:', data.length);
          ...
          if (conn.api_key_encrypted) {
            switch (conn.service_name) {
              ...
              case 'git':
                ...
                setGitRepositoryUrl(
                  'https://github.com/YourUsername/MyMindVentures.io.git'
                );
  ...
  const testConnection = async (
    ...
    try {
      ...
      const existingConnection = apiConnections.find(
        conn => conn.service_name === service
      );
      const updateData = {
      ...
      if (result.success) {
        ...
        alert(
          `❌ ${service.toUpperCase()} connection failed:\n${result.message}`
        );
      }
    } catch (error) {
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (pineconeKey.trim()) {
        savePromises.push(
          saveAPIKey('pinecone', pineconeKey, { environment: pineconeEnv })
        );
      }
      if (supabaseKey.trim()) {
      ...
      if (savePromises.length > 0) {
        alert(
          `✅ Successfully saved ${savePromises.length} API credential(s)!`
        );
      } else {
  ...
  const clearAPIKey = (serviceName: string) => {
    if (
      !confirm(
        `Are you sure you want to clear the ${serviceName.toUpperCase()} API key?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      // Clear from database
      const existingConnection = apiConnections.find(
        conn => conn.service_name === serviceName
      );
      if (existingConnection) {
        supabase
          .from('api_connections')
      ...
      alert(
        `✅ ${serviceName.toUpperCase()} credentials cleared successfully!`
      );
      loadAPIConnections();
    } catch (error) {
  ...
  const testAllConnections = async () => {
    ...
    const connections = [
      ...
      { service: 'perplexity', key: perplexityKey },
      {
        service: 'pinecone',
        key: pineconeKey,
        params: { environment: pineconeEnv },
      },
  ...
  const resetAllSettings = () => {
    if (
      confirm(
        'Are you sure you want to clear all API settings? This action cannot be undone.'
      )
    ) {
      try {
        // Clear all state
        ...
        <Card title='OpenAI Configuration' gradient>
          <div className='space-y-4'>
            <div className='flex items-center justify-between'>
              ...
              <div
                className={`flex items-center space-x-2 px-2 py-1 rounded text-xs ${getStatusColor(getConnectionStatus('openai'))}`}
              >
        ...
        <Card title='Perplexity Configuration'>
          <div className='space-y-4'>
            <div className='flex items-center justify-between'>
              ...
              <div
                className={`flex items-center space-x-2 px-2 py-1 rounded text-xs ${getStatusColor(getConnectionStatus('perplexity'))}`}
              >
                {getStatusIcon(getConnectionStatus('perplexity'))}
                <span className='capitalize'>
                  {getConnectionStatus('perplexity')}
                </span>
              </div>
            </div>
        ...
        <Card title='Pinecone Configuration'>
          <div className='space-y-4'>
            <div className='flex items-center justify-between'>
              ...
              <div
                className={`flex items-center space-x-2 px-2 py-1 rounded text-xs ${getStatusColor(getConnectionStatus('pinecone'))}`}
              >
                {getStatusIcon(getConnectionStatus('pinecone'))}
                <span className='capitalize'>
                  {getConnectionStatus('pinecone')}
                </span>
              </div>
            </div>
            ...
            <Button
              onClick={() =>
                testConnection('pinecone', pineconeKey, {
                  environment: pineconeEnv,
                })
              }
              disabled={
                !pineconeKey || !pineconeEnv || isTesting === 'pinecone'
              }
              className='w-full'
              size='sm'
            >
            ...
            <Button
              onClick={() =>
                saveAPIKey('pinecone', pineconeKey, {
                  environment: pineconeEnv,
                })
              }
        ...
        <Card title='Supabase Configuration'>
          <div className='space-y-4'>
            <div className='flex items-center justify-between'>
              ...
              <div
                className={`flex items-center space-x-2 px-2 py-1 rounded text-xs ${getStatusColor(getConnectionStatus('supabase'))}`}
              >
            ...
            <Button
              onClick={() =>
                testConnection('supabase', supabaseKey, { supabaseUrl })
              }
              disabled={
                !supabaseKey || !supabaseUrl || isTesting === 'supabase'
              }
              className='w-full'
              size='sm'
            >
            ...
            <Button
              onClick={() =>
                saveAPIKey('supabase', supabaseKey, { supabaseUrl })
              }
        ...
        <Card title='Git Configuration'>
          <div className='space-y-4'>
            <div className='flex items-center justify-between'>
              ...
              <div
                className={`flex items-center space-x-2 px-2 py-1 rounded text-xs ${getStatusColor(getConnectionStatus('git'))}`}
              >
            ...
            <Button
              onClick={() =>
                testConnection('git', gitAccessToken, {
                  repositoryUrl: gitRepositoryUrl,
                  defaultBranch: gitDefaultBranch,
                })
              }
              disabled={
                !gitAccessToken || !gitRepositoryUrl || isTesting === 'git'
              }
              className='w-full'
              size='sm'
            >
      ...
      <Card title='Connection Status Overview' gradient>
        <div className='grid grid-cols-1 md:grid-cols-5 gap-4'>
          {[
            ...
            const connection = apiConnections.find(
              conn => conn.service_name === service.key
            );
            ...
            return (
              ...
              >
                ...
                <div
                  className={`flex items-center space-x-2 px-2 py-1 rounded text-xs ${getStatusColor(status)}`}
                >
        ...
        <div className='mt-6 flex items-center justify-between'>
          <Button
            ...
            disabled={
              isLoading ||
              !apiConnections.some(conn => conn.api_key_encrypted?.trim())
            }
            variant='primary'
            className='px-6'
          >
          ...
          <div className='flex space-x-2'>
            ...
            <Button onClick={resetAllSettings} variant='danger' size='sm'>
              Reset All
            </Button>
          </div>
        </div>
