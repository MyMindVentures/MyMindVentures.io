...
interface CommitCreatorProps {
  ...
  onCreateCommit: (title: string, description: string, summary: string) => Promise<void>;
...
export const CommitCreator: React.FC<CommitCreatorProps> = ({
  ...
  const handleSubmit = async () => {
    if (!commitTitle.trim() || !commitDescription.trim() || !commitSummary.trim()) return;
          ...
          {pendingSnippetsCount === 0 && (
            <div className="p-3 bg-orange-500/10 border border-orange-500/20 rounded-lg">
              <div className="flex items-center space-x-2 text-orange-400 text-sm">
                ...
                <span>Create blueprint snippets first to generate commits.</span>
              </div>
            </div>
          )}
        </div>
        ...
        {generatedCommit && (
          <div className="space-y-3 p-4 bg-purple-500/10 border border-purple-500/20 rounded-lg">
            ...
            <Button 
              ...
              disabled={!commitTitle.trim() || !commitDescription.trim() || !commitSummary.trim() || isCreating}
              className="w-full"
              size="sm"
            >
              {isCreating ? (
