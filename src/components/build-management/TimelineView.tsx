...
export const TimelineView: React.FC<TimelineViewProps> = ({
  ...
  const handleDelete = async (item: TimelineItem) => {
    if (
      !confirm(
        `Are you sure you want to delete this ${item.type.replace('_', ' ')}?`
      )
    ) {
      return;
    }
  ...
  if (items.length === 0) {
    return (
      <div className='text-center py-8 text-gray-400'>
        No timeline items yet. Create your first blueprint snippet to get
        started.
      </div>
    );
  }
                ...
                <div className='flex items-center space-x-2'>
                  {viewingItem.content && (
                    <Button
                      variant='ghost'
                      size='sm'
                      onClick={() => handleCopyContent(viewingItem.content!)}
                    >
                  ...
                  <div className='grid grid-cols-2 gap-4 text-sm'>
                    <div>
                      ...
                      <span className='text-white ml-2'>
                        {format(new Date(viewingItem.timestamp), 'PPpp')}
                      </span>
                    </div>
                    {viewingItem.branch && (
                      <div>
                ...
                <div className='flex items-center space-x-3'>
                  ...
                  <div>
                    ...
                    <p className='text-gray-400 text-sm'>
                      Make changes to this timeline item
                    </p>
                  </div>
                </div>
                <button
                ...
                <div className='space-y-4'>
                  ...
                  {editingItem.type === 'publication' && (
                    <Input
                      ...
                      onChange={value =>
                        setEditForm(prev => ({ ...prev, version: value }))
                      }
                      placeholder='v1.0.0'
                    />
                  )}
                  ...
                  {(editingItem.content ||
                    editingItem.type === 'blueprint_snippet' ||
                    editingItem.type === 'commit' ||
                    editingItem.type === 'blueprint_file' ||
                    editingItem.type === 'special_page') && (
                    <div className='space-y-2'>
                      <label className='block text-sm font-medium text-gray-300'>
                        {editingItem.type === 'blueprint_snippet'
                          ? 'Snippet Content'
