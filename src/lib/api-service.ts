...
class APIService {
  ...
  async getAPIKey(
    service: 'openai' | 'perplexity' | 'pinecone' | 'supabase',
    userId: string = 'demo-user'
  ): Promise<{ key: string | null; metadata?: any }> {
    ...
    try {
      console.log(
        `Fetching ${service.toUpperCase()} API key from Supabase for user: ${userId}`
      );
      ...
      if (
        ...
        connections.api_key_encrypted.trim() === ''
      ) {
        console.log(
          `No valid ${service.toUpperCase()} API key found in database`
        );
        this.cache[service] = { key: null, lastFetch: Date.now() };
        return { key: null };
      }
  ...
  async callOpenAI(
    ...
    if (!apiKey) {
      throw new Error(
        'OpenAI API key not configured. Please save your OpenAI API key in App Settings first.'
      );
    }
  ...
  async callPerplexity(
    ...
    if (!apiKey) {
      throw new Error(
        'Perplexity API key not configured. Please save your Perplexity API key in App Settings first.'
      );
    }
  ...
  async queryPinecone(query: any, userId: string = 'demo-user') {
    ...
    if (!apiKey) {
      throw new Error(
        'Pinecone API key not configured. Please save your Pinecone API key in App Settings first.'
      );
    }
    ...
    if (!environment) {
      throw new Error(
        'Pinecone environment not configured. Please configure environment in App Settings.'
      );
    }
  ...
  async testAllConnections(userId: string = 'demo-user') {
    ...
    try {
      const { key: pineconeKey, metadata } = await this.getAPIKey(
        'pinecone',
        userId
      );
      if (pineconeKey && metadata?.environment) {
        const indexName = metadata.indexName || 'default';
        const response = await fetch(
          `https://${indexName}-${metadata.environment}.svc.pinecone.io/describe_index_stats`,
          {
            method: 'POST',
            headers: {
              'Api-Key': pineconeKey,
    ...
    try {
      const { key: supabaseKey, metadata } = await this.getAPIKey(
        'supabase',
        userId
      );
...
export const openaiService = {
  getAPIKey: (userId?: string) =>
    apiService.getAPIKey('openai', userId).then(result => result.key),
  callOpenAI: (messages: any[], options?: any, userId?: string) =>
    apiService.callOpenAI(messages, options, userId),
