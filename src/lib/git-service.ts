...
class GitService {
  ...
  private async makeGitApiRequest(endpoint: string, options: RequestInit = {}) {
    ...
    if (!response.ok) {
      throw new Error(`Git API request failed: ${response.status} ${response.statusText}`);
    }
  ...
  private async getRemoteCommits(): Promise<GitCommit[]> {
    ...
    const endpoint = url.hostname === 'github.com' 
  ...
  async createRemoteBranch(branchName: string, sourceBranch: string = 'main') {
    ...
    try {
      ...
      const branchData = url.hostname === 'github.com' ? {
      ...
      if (uncommittedFiles && uncommittedFiles.length > 0) {
        ...
        if (result.success) {
          ...
          await supabase
            ...
            .eq('user_id', this.userId)
            .in('path', uncommittedFiles.map(f => f.path));
        }
